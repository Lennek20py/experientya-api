    ...
    <div v-if="!loadData"  role="status" class="sm:max-w-sm lg:max-w-full animate-pulse pb-5 lg:pb-0">
     ...
     <div v-if="!formBind && loadData && !test_finished" class="flex justify-center align-center flex-col my-1">
        ...
        <div @click="toggleModal()" class="w-auto flex justify-center items-center lg:justify-center lg:py-1">
            <a v-if="ifExists" class="flex items-center cursor-pointer content-center justify-cente text-lg font-semibold text-start rounded-full bg-red-600 text-gray-50 px-4 py-1">
            CONTINUAR</a>
            <a v-else class="flex items-center cursor-pointer content-center justify-cente text-lg font-semibold text-start rounded-full bg-red-600 text-gray-50 px-4 py-1">
            INICIAR TEST</a>
        </div>
    </div>
  ...
  <div class="z-10 bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-7xl sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
      ...
      <div class="sm:flex w-full sm:items-start">
        <div class="mt-3 w-full text-center sm:mt-0 sm:ml-4 sm:text-left overflow-auto ...">
          <div class="flex text-center flex-col justify-center items-center mb-4 w-full">
                <!-- TÍTULO -->
            <h3 class="uppercase px-1 py-2 text-xl text-center leading-6 font-bold text-gray-900 lg:text-3xl" id="modal-headline">
              TEST DE COMPETENCIAS
            </h3>
                <!-- INSTRUCCIONES -->
            <p class="text-xs text-gray-500 text-justify border-b-2 sm:mx-10 md:mx-20 lg:text-base">Lea atentamente el enunciado y seleccione la opción que más se acople a su manera de ser. Si desea cerrar la ventana del test, este se guardará en la última respuesta brindada; le recomendamos tomar el test sin distractores externos y en un solo intento.</p>
          </div>
            <!-- CUERPO -->
    ...
    <div class="bg-gray-50 gap-2 px-4 py-6 sm:px-6 flex flex-col sm:flex-row-reverse">
      <span class="mt-3 flex w-full rounded-md shadow-sm sm:mt-0 sm:w-auto">
        <button @click="toggleModalTest(), toggleModal()" type="button" class="inline-flex justify-center font-bold w-full rounded-md select-none border border-gray-300 px-4 py-2 bg-red-600 text-base leading-6 text-white shadow-sm hover:text-gray-2000 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue transition ease-in-out duration-150 sm:text-sm sm:leading-5">
    ...
    <div class="bg-white px-4 pt-2 pb-4 sm:p-5 sm:pb-5 flex-col flex">
      ...
      <div class="sm:flex w-full sm:items-start">
        <div class="mt-3 w-full text-center sm:mt-0 sm:ml-4 sm:text-left overflow-auto ...">
          <div class="flex text-center flex-col justify-center items-center mb-4 w-full">
                <!-- TÍTULO -->
            <h3 class="uppercase px-1 py-2 pt-0 text-2xl text-center leading-6 font-extrabold text-gray-900" id="modal-headline">
              TEST DE COMPETENCIAS
            </h3>
                <!-- INSTRUCCIONES -->
            <p v-if="!this.isFinishedTest" class="text-xs select-none text-gray-500 text-justify border-b-2 lg:mx-10 lg:text-base">Lea atentamente el enunciado y seleccione la opción que más se acople a su manera de ser. Si desea cerrar la ventana del test, se reiniciará por completo, por lo que, le pedimos atentamente que el test sea iniciado y terminado en el mismo intento.
            </p>
            <p v-else class="text-xs text-gray-500 select-none text-justify border-b-2 lg:text-lg">¡Gracias por realizar el test de competencias!, los resultados han sido guardados y están en espera a revisión. Puede continuar completando el resto de zonas para su CV.</p>
          </div>
            <!-- CUERPO -->
        ...
        <!-- BOTONES / FOOTER PAGE -->
    <div class="flex flex-col bg-gray-50 px-4 py-6 text-center items-center justify-between content-center sm:px-6 lg:items-end lg:flex-row lg:text-end">
      <p v-if="!this.isFinishedTest" class="order-last mt-4 text-gray-900 text-base items-end lg:text-lg lg:order-first">{{this.countQuestion+1}}/28</p>
      <span v-if="!this.isFinishedTest" class="mt-3 flex w-auto items-center rounded-md justify-between shadow-sm sm:mt-0 sm:w-auto lg:align-middle">
      ...
      <span v-else class="mt-3 flex w-auto items-center align-middle rounded-md shadow-sm sm:mt-0 sm:w-auto lg:self-end">
        <button @click="isStartTest = false, ifExists = true" type="button" class=" inline-flex justify-center justify-items-end w-full rounded-md border border-gray-300 mx-2 px-4 py-2 bg-blue-600 text-base leading-6 font-medium text-white shadow-sm hover:text-white focus:outline-none focus:border-blue-300 hover:bg-blue-700 focus:shadow-outline-blue transition ease-in-out duration-150 sm:text-sm sm:leading-5 lg:px-8 lg:py-3 lg:text-xl">
          Finalizar
        </button>
      </span>
    </div>
