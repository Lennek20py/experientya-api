...
<div class="w-full w-4xl bg-white rounded-lg border border-gray-200 shadow-m px-5 py-2 2xl:px-8 2xl:min-h-[204px] transition ease-in-out delay-75 hover:-translate-x-1">
    ...
    <div class="2xl:mb-4" v-else-if="formBind && loadData">
            <div class="">
                <p id="helper-text-explanation" class=" text-[10px] text-justify lg:text-xs text-gray-500">¿No cuentas con experiencia laboral?, no te preocupes. Puedes ingresar experiencia como becario o como estudiante y referenciar a tus maestros.</p>
            ...
            <form @submit.prevent="submit">
                <div class="flex flex-col">
                    <!-- PUESTO INPUT / JOB -->
                    <div class="mt-3 grow">
                        <label for="studyLevel" class="text-xs text-gray-800 font-medium lg:text-sm 2xl:text-base after:text-red-500 after:content-['*']">Puesto</label>
                        <input type="text" v-model="this.experience.job" required placeholder="Ingrese un puesto de trabajo." name="job" id="job" class="mt-1 block w-full text-xs rounded-md border-gray-300 py-2 pl-3 lg:pr-10 focus:border-blue-500 focus:outline-none focus:ring-blue-500 xl:text-base">
                    </div>
                    ...
                    <div class="flex flex-row justify-around items-center align-center grow gap-2 lg:gap-4">
                        <div class="flex w-full 2xl:ml-1 self-start">
                            <div class="mt-3 grow">
                                <label for="study_status" class="text-xs text-gray-800 font-medium lg:text-sm 2xl:text-base after:text-red-500 after:content-['*']">Fecha de Inicio</label>
                                <input v-model="this.experience.start_date" type="date" name="start_date" id="start_date" required class="mt-1 block w-full text-xs rounded-md border-gray-300 py-1 pl-3 lg:pr-10 focus:border-blue-500 focus:outline-none focus:ring-blue-500 xl:text-base"/>
                            </div>
                        </div>
                        <div class="flex w-full gap-2 2xl:ml-4 lg:gap-4">
                            <div class="mt-3 grow">
                                <label for="end_date" v-bind:class="{ disabled: this.experience.actual_status == 1 }" class="text-xs text-gray-800 font-medium lg:text-sm 2xl:text-base after:text-red-500 after:content-['*']">Fecha de Finalización</label>
                                <input v-model="this.experience.end_date" v-if="this.experience.actual_status == 0" placeholder="dd/mm/yyyy" type="date" name="end_date" id="end_date" required class="mt-1 block w-full text-xs rounded-md border-gray-300 py-1 pl-3 lg:pr-10 focus:border-blue-500 focus:outline-none focus:ring-blue-500 xl:text-base"/>
                                <div class="flex flex-row justify-end align-center pt-px">
                                    <label for="actual" class="text-[10px] text-justify lg:text-xs text-gray-500 align-center">
                                    <input @change="watchStatus()" type="checkbox" name="actual" id="actual" class="w-3 h-3 mr-px rounded-sm" v-model="this.experience.actual_status" true-value="1" false-value="0">
                                        Actual
                                    </label>
                                </div>
                            </div>
                ...
                <!-- EMPRESA FIELD -->
                <div class="mt-0 grow">
                    <label for="company" class="text-xs text-gray-800 font-medium lg:text-sm 2xl:text-base after:text-red-500 after:content-['*']">Empresa</label>
                    <input required type="text" v-model="this.experience.company" placeholder="Ingrese la empresa." name="company" id="company" class="mt-1 block w-full text-xs rounded-md border-gray-300 py-2 pl-3 lg:pr-10 focus:border-blue-500 focus:outline-none focus:ring-blue-500 xl:text-base">
                </div>
                <div class="flex flex-row justify-around items-start grow gap-2 lg:gap-4">
                    <div class="flex grow 2xl:ml-1 basis-1/3">
                        <div class="mt-3 grow">
                            <label for="country" class="text-xs text-gray-800 font-medium lg:text-sm 2xl:text-base">País</label>
                            <select @change="fetchStates()" v-model="this.experience.country" required name="country" id="country" class="mt-1 block w-full text-xs rounded-md border-gray-300 py-2 pl-3 lg:pr-10 focus:border-blue-500 focus:outline-none focus:ring-blue-500 xl:text-base">
                    ...
                    <div class="flex grow basis-1/3 2xl:ml-4">
                        <div class="mt-3 grow">
                            <label for="study_certificates" class="text-xs text-gray-800 font-medium lg:text-sm 2xl:text-base">Estado</label>
                            <select @change="fetchCities()" v-model="this.experience.state" required name="study_certificate" id="study_certificate" class="mt-1 block w-full text-xs rounded-md border-gray-300 py-2 pl-3 lg:pr-10 focus:border-blue-500 focus:outline-none focus:ring-blue-500 xl:text-base">
                    ...
                    <div class="flex grow basis-1/3 2xl:ml-4">
                        <div class="mt-3 grow">
                            <label for="study_certificate" class="text-xs text-gray-800 font-medium lg:text-sm 2xl:text-base">Ciudad / Municipio</label>
                            <select v-model="this.experience.city" required name="study_certificate" id="study_certificate" class="mt-1 block w-full text-xs rounded-md border-gray-300 py-2 pl-3 lg:pr-10 focus:border-blue-500 focus:outline-none focus:ring-blue-500 xl:text-base">
                ...
                <div class="mt-3 grow">
                    <label for="studyLevel" class="text-xs text-gray-800 font-medium lg:text-sm 2xl:text-base after:text-red-500 after:content-['*']">Referencia de la empresa</label>
                    <textarea required v-model="this.experience.company_description" name="company_description" id="company_description" rows="4" class="block p-2.5 w-full text-xs text-gray-900 bg-white rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="Nombre del responsable, teléfono y correo electrónico de contacto..."></textarea>
                </div>
            ...
            <div class="flex mt-3 w-full flex-col justify-between items-center lg:flex-row 2xl:py-3">
                <div class="flex flex-wrap justify-center w-full order-first lg:order-last lg:w-auto lg:justify-end ">
                    <jet-button v-if="formBind" :disabled="experience.processing" class="rounded-lg bg-blue-500 text-gray-50 text-base flex!important text-center lg:mx-2 items-center font-semibold px-5 py-1 w-full lg:w-auto">Guardar</jet-button>
    ...
    <div v-else-if="!formBind && ifExists && loadData" class="flex flex-col">
        <div @click="edit(Experience)" v-for="(Experience, index) in experiences" :key="index"  class="w-full flex flex-col items-center text-center mb-3 md:mb-5 lg:items-start lg:text-start border-b border-gray-200 group in-ease-out delay-150 cursor-pointer">
            ...
            <div class="flex flex-nowrap justify-center items-center py-0 transition">
                <svg class="min-w-fit w-4 h-4 lg:w-5 lg:h-5 self-start text-gray-300 pr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
            ...
            <div class="flex flex-nowrap justify-center items-center py-0 transition">
                ...
                <p class="text-xs text-gray-500">{{Experience.city}}, {{Experience.state}}, {{Experience.country}}</p>
            </div>
            <div class="flex flex-col w-auto justify-center items-center lg:flex lg:flex-row lg:justify-between lg:w-full">
                <div class="flex flex-nowrap justify-center items-center py-2 transition">
                    ...
                    <div class="text-sm text-gray-600 text-center w-auto leading-none align-middle">{{Experience.job}}</div>
                </div>
         ...
         <div class="w-auto flex justify-center items-center lg:justify-center lg:py-4">
            <a @click="newDataMethod()" class="lg:hiddn flex items-center cursor-pointer content-center justify-center text-red-500 text-lg font-semibold text-start lg:rounded-full lg:bg-red-500 lg:text-gray-50 lg:px-4 lg:py-1">
...
export default {
    ...
    methods: {
        ...
        // async fetchLocationInfo(id) {
        //     // console.log("holos");
        //     await axios.get(route('search-location-info', id))
        //     .then((response) => {
