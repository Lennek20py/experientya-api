    ...
    <template #header>
      <div class="2 mb-12">
        <div>
          ...
          <div class="flex justify-end container-btn-update-banner p-2">
            <button
              type="button"
              class="text-white bg-blue-700 hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 p-2 relative overflow-hidden ml-2"
              @click="changeImg(2)"
            >
              <svg
          ...
          <div class="flex justify-center container-img">
            <div
              class="relative w-40 h-40 overflow-hidden bg-gray-100 rounded-full dark:bg-gray-600 border-2 border-solid"
            >
              <img
                class="w-full h-full object-cover"
                :src="photoProfile"
      ...
      <tabs :arrayButtons="arrayButtons" class="mb-12">
        ...
        <template #tabTwo>
          <div>
            <span>
              Asegúrese de que su cuenta esté utilizando una contraseña larga y
        ...
        <template #tabThree>
          <div>
            <span>
              Agregue seguridad adicional a su cuenta utilizando la
        ...
        <template #tabFour>
          <div>
            <span>
              Administre e inicie sesión en sus sesiones activas en otros
        ...
        <template #tabFive>
          <div>
            <span> Elimina permanentemente su cuenta. </span>
      ...
      <transition name="fade">
        ...
        <ModalComponent
          ...
          :styleModal="'max-w-2xl lg:w-2/6 w-full'"
          @closeMod="closeModal(), resetImg(1)"
          @clickSave="sendPhoto(1)"
        >
          <template #contenido>
            <div>
        ...
        <ModalComponent
          ...
          :title="'Banner'"
          :styleModal="'max-w-2xl lg:w-2/6 w-full'"
          @closeMod="closeModal(), resetImg(2)"
          @clickSave="sendPhoto(2)"
        >
          <template #contenido>
            <div>
              <div class="flex justify-center">
        ...
        <ModalComponent
          ...
          :textBtnSave="'Eliminar'"
          :styleModal="'max-w-5xl lg:w-5/6 w-full'"
          @closeMod="closeModal()"
          @clickSave="deleteAcount"
        >
          <template #contenido>
            <div class="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <span class="text-sm text-inherit"
                  >Ayudamos con las siguientes preguntas antes de eliminar tu
                  cuenta</span
                >
                <div class="mt-6">
                  <div class="mt-3">
                    <div class="z-10 mb-[-10px]">
                      <span
                        class="text-sm font-medium text-slate-700 after:ml-0.5 ml-3 bg-white pl-2 pr-2 focus:ring-blue-500"
...
// image profile path
let photoProfile = ref(
  props.user.profile_photo_path.replace("profile/", "storage/profile/")
);

// image banner path
let photoBanner = ref(
  props.user.banner_photo_path.replace("banner/", "storage/banner/")
);
const newPhoto =
  props.user.banner_photo_path === "banner/banner-default.png" ? false : true;
...
let arrayButtons = ref([
  {
    ...
    value: 1,
    class: "border-current text-teal-600",
    referencia: "tabOne",
  },
...
function onFileSelected(e) {
  if (e.target.files[0]) {
    ...
    photoProfile.value = props.user.profile_photo_path.replace(
      "profile/",
      "storage/profile/"
    );
  }
...
function onFileSelectedBanner(e) {
  if (e.target.files[0]) {
    ...
    photoBanner.value = props.user.banner_photo_path.replace(
      "profile/",
      "storage/profile/"
    );
  }
