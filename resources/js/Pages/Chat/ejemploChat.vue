    ...
    <html lang="en">
        ...
        <body>
            <section class="msger">
                ...
                <div ref="msgerChat" class="msger-chat"></div>
    ...
    await axios
        ...
        .then((res) => {
            ...
            appendMessage(
                ...
                data.user.profile_photo_path.replace(
                    "profile/",
                    "storage/profile/"
                ),
                "right",
...
function appendMessage(name, img, side, text, date) {
    ...
    const borderRadius =
        side === "right"
            ? "rounded-t-[15px] rounded-l-[15px] mr-2"
            : "rounded-t-[15px] rounded-r-[15px] ml-2";
    const backgColor = side === "right" ? "bg-blue-400" : "bg-slate-400";
    //   Simple solution for small apps
    const msgHTML = `
  <div class="flex items-end mb-[10px] ${direction}">
    <img class="w-[50px] h-[50px] mr-[10px] bg-no-repeat bg-center bg-cover rounded-[50%] bg-zinc-400" src="${img}"/>

    <div class="max-w-[450px] p-[15px] ${borderRadius} ${backgColor} ">
      <div class="flex justify-between items-center mb-[10px]">
        <div class="mr-[10px] font-bold">${name}</div>
        <div class="text-[0.85em]">${date}</div>
      </div>
